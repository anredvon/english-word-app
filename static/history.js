...
(() => {
  ...
  async function load(){shell();const status=document.getElementById('historyStatus');try{
    ...
    const [sr,hr,wr]=await Promise.allSettled([fetch(`/api/stats/daily?from=${from}&to=${to}`),fetch(`/api/history?from=${from}&to=${to}`),fetch('/api/words')]);
    const sres=sr.status==='fulfilled'?sr.value:null,hres=hr.status==='fulfilled'?hr.value:null,wres=wr.status==='fulfilled'?wr.value:null;
    let stats=[],history=[],words=[];
    if(sres?.ok){const x=await sres.json();stats=Array.isArray(x)?x:[]}
    if(hres?.ok){const x=await hres.json();history=Array.isArray(x)?x:[]}
    if(wres?.ok){const x=await wres.json();words=Array.isArray(x)?x:[]}
    text('historyTotalWords',words.length);
