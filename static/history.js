...
(() => {
  ...
  async function load(){shell();const status=document.getElementById('historyStatus');try{
    ...
    text('historyTotalWords',words.length);const growth=window.DinoLearning.growth(masteryWords),mastered=growth.mastered,growing=growth.growing,needReview=masteryWords.filter(window.DinoLearning.needsReview).length,learned=growth.learned; text('historyMastered',mastered);text('historyGrowing',growing);text('historyNeedReview',needReview);const mf=document.getElementById('historyMasteryFill');if(mf)mf.style.width=learned?`${Math.round(mastered/learned*100)}%`:'0%';text('historyMasteryMessage',learned?`${learned}개 단어를 학습했고, 그중 ${mastered}개를 완전 암기 단계까지 키웠어요.`:'첫 학습을 시작하면 단어의 기억 성장이 여기에 쌓여요.');
